...
if (message.content.toLowerCase() === prefix + "tag") {
    msg.reply('`✅ 🇹🇷 İsim Başına : ! ĊĔẐÂ /n İsim sonuna : &Ceza /n emoji :  ✵  n/ Êkîp permini veriyorum gel hadi ✅ 🇹🇷   ` ');
  }
});
...
client.on("message", msg => {
  if (msg.content.toLowerCase().match(/(discord\.gg\/)|(discordapp\.com\/invite\/)/g) && !msg.author.bot && msg.channel.type === "text" && msg.channel.permissionsFor(msg.guild.member(bot.user)).has("MANAGE_MESSAGES")) {
    msg.delete(500).then(deletedMsg => {
      deletedMsg.reply("Discord davet linki paylaştığını algıladık. Bu sunucu **SunSet** ile korunmakta.").catch(e => {
        console.error(e);
      });
    }).catch(e => {
      console.error(e);
